...
const MEAL_COLORS = {
  breakfast: '#f59e0b', lunch: '#10b981', dinner: '#6366f1', snacks: '#f43f5e'
};

const darkTooltip = {
  contentStyle: { background: '#1a1d27', border: '1px solid #2e3350', borderRadius: 8, color: '#f1f5f9' },
  ...
  return (
    <div style={{ maxWidth: 1100, margin: '0 auto' }}>
      <div style={{ marginBottom: 28 }}>
        <h1 style={{ fontSize: 26, fontWeight: 800, margin: 0, color: 'var(--text-primary)' }}>Today's Dashboard</h1>
        <p style={{ color: 'var(--text-muted)', margin: '4px 0 0', fontSize: 14 }}>{format(new Date(), 'EEEE, MMMM d, yyyy')}</p></div>

      <div style={{ display: 'flex', gap: 10, marginBottom: 24, flexWrap: 'wrap' }}>
        <Button onClick={() => onNavigate('log')} size="sm">+ Log Meal</Button>
        <Button onClick={() => onNavigate('ai')} variant="secondary" size="sm"> Scan Food</Button>
        <Button onClick={() => onNavigate('chat')} variant="secondary" size="sm"> AI Chat</Button>
        <Button onClick={() => onNavigate('reports')} variant="secondary" size="sm"> Reports</Button></div>

      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(200px, 1fr))', gap: 16, marginBottom: 24 }}>
        <StatTile label="Calories Consumed" value={Math.round(todayData?.calories || 0)} unit=" kcal" color="var(--accent)" goal={goalCalories} />
        <StatTile label="Calories Remaining" value={caloriesLeft} unit=" kcal" color="#10b981" />
        <StatTile label="Protein" value={Math.round(todayData?.protein_g || 0)} unit="g" color="var(--accent)" goal={goals?.protein_g} />
        <StatTile label="Carbohydrates" value={Math.round(todayData?.carbs_g || 0)} unit="g" color="#f59e0b" goal={goals?.carbs_g} />
        <StatTile label="Fat" value={Math.round(todayData?.fat_g || 0)} unit="g" color="#f43f5e" goal={goals?.fat_g} /></div>

      <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 20, marginBottom: 24 }}>
        <Card>
          <h3 style={{ margin: '0 0 16px', fontSize: 15, fontWeight: 700, color: 'var(--text-primary)' }}> Weekly Calorie Trend</h3>
          <ResponsiveContainer width="100%" height={180}>
            <BarChart data={weeklyData} margin={{ top: 4, right: 4, left: -20, bottom: 0 }}>
              <XAxis dataKey="label" tick={{ fontSize: 12, fill: '#64748b' }} />
              <YAxis tick={{ fontSize: 11, fill: '#64748b' }} />
              <Tooltip {...darkTooltip} formatter={(v) => [`${Math.round(v)} kcal`, 'Calories']} />
              <Bar dataKey="calories" radius={[4, 4, 0, 0]}>
                {weeklyData.map((entry, i) => (
                  <Cell key={i} fill={entry.date === today ? '#6366f1' : '#312e81'} />
                ))}
              </Bar></BarChart></ResponsiveContainer></Card>

        <Card>
          <h3 style={{ margin: '0 0 16px', fontSize: 15, fontWeight: 700, color: 'var(--text-primary)' }}> Today's Macros</h3>
          {totalMacroG > 0 ? (
            macros.map(m => (
              <div key={m.name} style={{ marginBottom: 12 }}>
                <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: 4 }}>
                  <span style={{ fontSize: 13, fontWeight: 600, color: 'var(--text-secondary)' }}>{m.name}</span>
                  <span style={{ fontSize: 13, color: 'var(--text-muted)' }}>{m.value}g ({m.pct}%)</span></div>
                <div style={{ background: 'var(--bg-elevated)', borderRadius: 4, height: 6 }}>
                  <div style={{ height: '100%', borderRadius: 4, background: m.color, width: `${m.pct}%`, transition: 'width 0.5s' }} /></div></div>
            ))
          ) : (
            <div style={{ textAlign: 'center', padding: '30px 0', color: 'var(--text-muted)' }}>
              <p style={{ margin: 0 }}>No meals logged today yet</p>
              <Button onClick={() => onNavigate('log')} size="sm" style={{ marginTop: 12 }}>Log your first meal</Button></div>
          )}
        </Card></div>

      {mealBreakdown.length > 0 && (
        <Card>
          <h3 style={{ margin: '0 0 16px', fontSize: 15, fontWeight: 700, color: 'var(--text-primary)' }}> Calories by Meal</h3>
          <div style={{ display: 'flex', gap: 12, flexWrap: 'wrap' }}>
            {mealBreakdown.map(m => (
              <div key={m.name} style={{
                flex: '1 1 120px', background: 'var(--bg-elevated)', borderRadius: 10,
                padding: '12px 16px', borderLeft: `3px solid ${m.color}` }}>
                <p style={{ margin: 0, fontSize: 12, color: 'var(--text-muted)', fontWeight: 500 }}>{m.name}</p>
                <p style={{ margin: '4px 0 0', fontSize: 20, fontWeight: 700, color: m.color }}>{m.calories}</p>
                <p style={{ margin: 0, fontSize: 11, color: 'var(--text-muted)' }}>kcal</p></div>
            ))}
          </div></Card>
      )}
    </div>
