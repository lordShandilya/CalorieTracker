    ...
    <div style={{ maxWidth: 900, margin: '0 auto' }}>
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: 24 }}>
        <div>
          ...
          <p style={{ color: 'var(--text-muted)', margin: '4px 0 0', fontSize: 14 }}>Track your daily food intake</p></div>
        <Button onClick={openCreate}>+ Log Meal</Button></div>
      ...
      <Card style={{ marginBottom: 20 }}>
        <div style={{ display: 'flex', gap: 12, flexWrap: 'wrap', alignItems: 'flex-end' }}>
          <div style={{ flex: '1 1 160px' }}>
            <label style={{ fontSize: 12, fontWeight: 600, color: 'var(--text-muted)', display: 'block', marginBottom: 4 }}>Filter by Date</label>
            <Input type="date" value={filterDate} onChange={e => { setFilterDate(e.target.value); setPage(1); }} /></div>
          <div style={{ flex: '1 1 140px' }}>
            <label style={{ fontSize: 12, fontWeight: 600, color: 'var(--text-muted)', display: 'block', marginBottom: 4 }}>Meal Type</label>
          ...
          <div style={{ flex: '1 1 160px' }}>
            <label style={{ fontSize: 12, fontWeight: 600, color: 'var(--text-muted)', display: 'block', marginBottom: 4 }}>Date Range From</label>
            <Input type="date" value={startDate} onChange={e => { setStartDate(e.target.value); setFilterDate(''); setPage(1); }} /></div>
          <div style={{ flex: '1 1 160px' }}>
            <label style={{ fontSize: 12, fontWeight: 600, color: 'var(--text-muted)', display: 'block', marginBottom: 4 }}>To</label>
            <Input type="date" value={endDate} onChange={e => { setEndDate(e.target.value); setFilterDate(''); setPage(1); }} /></div>
      ...
      {loading ? (
        ...
        <Card style={{ textAlign: 'center', padding: 48 }}>
          ...
          <p style={{ color: 'var(--text-muted)', margin: 0 }}>No meals logged yet. Click "Log Meal" to get started!</p></Card>
      ) : (
        sortedDates.map(date => (
          <div key={date} style={{ marginBottom: 24 }}>
            <h3 style={{ fontSize: 14, fontWeight: 700, color: 'var(--text-secondary)', margin: '0 0 12px', textTransform: 'uppercase', letterSpacing: 0.5 }}>
                ...
                {grouped[date][meal].map(entry => (
                  <div key={entry.id} style={{
                    ...
                    <div style={{ display: 'flex', gap: 12, fontSize: 12, color: 'var(--text-muted)' }}>
                      ...
                      <span>F: {Math.round(entry.fat_g || 0)}g</span></div>
                    <div style={{ display: 'flex', gap: 6 }}>
                      <Button size="sm" variant="ghost" onClick={() => openEdit(entry)}></Button>
                      <Button size="sm" variant="ghost" onClick={() => handleDelete(entry.id)} style={{ color: '#ef4444' }}></Button></div></div>
                ))}
              </Card>
            ))}
          </div>
        ...
        <form onSubmit={handleSave}>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 12 }}>
            <Field label="Food Name" style={{ gridColumn: '1/-1' }}>
              <Input value={form.food_name} onChange={setF('food_name')} placeholder="e.g. Grilled Chicken Breast" required /></Field>
            ...
            <Field label="Date">
              <Input type="date" value={form.entry_date} onChange={setF('entry_date')} required /></Field>
            <Field label="Quantity">
              <Input type="number" value={form.quantity} onChange={setF('quantity')} placeholder="e.g. 200" min="0" step="any" required /></Field>
            <Field label="Unit">
          ...
          <h4 style={{ margin: '16px 0 12px', fontSize: 13, fontWeight: 700, color: 'var(--text-secondary)', textTransform: 'uppercase', letterSpacing: 0.5 }}>
            Macronutrients
          </h4>
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(4, 1fr)', gap: 10 }}>
            {[['calories', 'kcal'], ['protein_g', 'Protein (g)'], ['carbs_g', 'Carbs (g)'], ['fat_g', 'Fat (g)']].map(([k, label]) => (
              <Field key={k} label={label}>
                <Input type="number" value={form[k]} onChange={setF(k)} placeholder="0" min="0" step="any" /></Field>
            ))}
          </div>
          ...
          {showAdvanced && (
            <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3, 1fr)', gap: 10 }}>
              ...
              <Field label="Notes" style={{ gridColumn: '1/-1' }}>
                <Input value={form.notes} onChange={setF('notes')} placeholder="Optional notes..." /></Field></div>
          )}
